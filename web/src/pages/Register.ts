...
// Logique séparée pour être appelée par le routeur
export function initRegisterPage() {
  ...
  form.addEventListener('submit', async function(event) {
      ...
      try {
          // Utilisation du chemin relatif pour passer par le proxy (Vite ou Nginx)
          const response = await fetch('/back_to_back/register', {
              method: 'POST',
              headers: {
          ...
          if (response.ok) {
              ...
              alert('Erreur lors de l\'inscription: ' + (result.message || result.error));
          }
      } catch (error) {
